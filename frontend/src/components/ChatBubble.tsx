...
const ChatBubble = ({ message, previousMessage }: ChatBubbleProps) => {
  ...
  const isMember = selectedConversation?.participants.includes(message.user) || false;
  const isGroup = selectedConversation?.is_group;
  const fromMe = message.user?.id === me?.id;
  const fromAI = false;
      ...
      <>
        ...
        <div className='flex gap-1 w-2/3'>
          ...
          <div className={`flex flex-col z-20 max-w-fit px-2 pt-1 rounded-md shadow-md relative ${bgClass}`}>
            ...
            {/* {open && <ImageDialog src={message.text} open={open} onClose={() => setOpen(false)} />} */}
...
const TextMessage = ({ message }: { message: IMessage }) => {
  const isLink = /^(ftp|http|https):\/\/[^ "]+$/.test(message.text);
